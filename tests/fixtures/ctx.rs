...
pub struct E2ETurmoilCtx {
    pg: Option<ContainerAsync<Postgres>>,
    pub nodes: Vec<test_helpers::TurmoilProcess>,
    clients: Vec<NodeApiHttpClient>,
    client_index: usize,
    indexer_client: Option<IndexerApiHttpClient>,
    slot_duration: u64,
...
impl E2ETurmoilCtx {
    ...
    fn build_nodes(
        count: usize,
        conf_maker: &mut ConfMaker,
    ) -> (Vec<test_helpers::TurmoilProcess>, Vec<NodeApiHttpClient>) {
        let mut nodes = Vec::new();
        let mut clients = Vec::new();
        let mut peers = Vec::new();
        let mut confs = Vec::new();
        ...
        for i in 0..count {
            ...
            node_conf.p2p.peers = peers.clone();
            genesis_stakers.insert(node_conf.id.clone(), 100);
            peers.push(node_conf.p2p.address.clone().to_string());
            confs.push(node_conf);
        }
        ...
        for node_conf in confs.iter_mut() {
            ...
            let node = test_helpers::TurmoilProcess {
                conf: conf_clone.clone(),
                client: NodeApiHttpClient {
                    url: Url::parse(&format!("http://{}", &conf_clone.rest_address)).unwrap(),
                    reqwest_client: Client::new(),
                    api_key: None,
                },
            };
    ...
    pub async fn new_single(slot_duration: u64) -> Result<E2ETurmoilCtx> {
        ...
        let node = test_helpers::TurmoilProcess {
            ...
            client: NodeApiHttpClient {
                url: Url::parse(&format!("http://{}", &node_conf.rest_address)).unwrap(),
                reqwest_client: Client::new(),
                api_key: None,
            },
        };
    ...
    pub fn new_multi(count: usize, slot_duration: u64) -> Result<E2ETurmoilCtx> {
        ...
        Ok(E2ETurmoilCtx {
            ...
            clients,
            client_index: 0,
            indexer_client: Some(IndexerApiHttpClient::new(indexer.to_string())?),
            slot_duration,
        })
    }
