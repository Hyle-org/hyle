...
pub mod risc0 {
    ...
    impl<'a> Risc0Prover<'a> {
        ...
        pub async fn prove(&self, program_inputs: ProgramInput) -> Result<ProofData> {
            let explicit = std::env::var("RISC0_PROVER").unwrap_or_default();
            let receipt = match explicit.to_lowercase().as_str() {
                "bonsai" => {
                    let program_inputs = bonsai_runner::as_input_data(&program_inputs)?;
                    bonsai_runner::run_bonsai(self.binary, program_inputs.clone()).await?
                }
                _ => {
                    let program_inputs = borsh::to_vec(&program_inputs)?;
                    let env = risc0_zkvm::ExecutorEnv::builder()
                        .write(&program_inputs.len())?
                        .write_slice(&program_inputs)
                        .build()
                        .unwrap();
    ...
    impl ClientSdkProver for Risc0Prover<'_> {
        fn prove(
            ...
            Box::pin(self.prove(program_inputs))
        }
    }
}
...
pub mod sp1 {
    ...
    pub fn execute(binary: &[u8], program_inputs: &ProgramInput) -> Result<HyleOutput> {
        let client = ProverClient::from_env();
        let mut stdin = SP1Stdin::new();
        let encoded = borsh::to_vec(program_inputs)?;
...
pub mod test {
    ...
    impl ClientSdkProver for TestProver {
        fn prove(
            ...
            Box::pin(async move {
                let hyle_output = test::execute(&program_inputs)?;
                Ok(ProofData(borsh::to_vec(&vec![hyle_output])?))
            })
        }
    }

    pub fn execute(program_inputs: &ProgramInput) -> Result<HyleOutput> {
        // FIXME: this is a hack to make the test pass.
        let initial_state = StateDigest(program_inputs.serialized_initial_state.clone());
