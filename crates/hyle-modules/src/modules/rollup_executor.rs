...
pub struct RollupExecutorStore<Contract> {
    ...
    contract: Contract,
    start_executing_at: BlockHeight,
        ...
        + 'static,
{
    ...
    async fn build(bus: SharedMessageBus, ctx: Self::Context) -> Result<Self> {
        ...
        let store = match Self::load_from_disk::<RollupExecutorStore<Contract>>(file.as_path()) {
            ...
            None => RollupExecutorStore::<Contract> {
                ...
                unsettled_txs: vec![],
                state_history: vec![],
                start_executing_at: ctx.start_height,
                proved_height: ctx.start_height,
            },
        };

        Ok(RollupExecutor { bus, store, ctx })
    }
    ...
    async fn run(&mut self) -> Result<()> {
        ...
        self.store.start_executing_at = self.store.proved_height;
    ...
    Contract: TxExecutorHandler + Debug + Clone + Send + Sync + 'static,
{
    ...
    fn prove_supported_blob(
        ...
        for (blob_index, tx, tx_ctx) in blobs {
            let old_tx = tx_ctx.block_height.0 < self.store.start_executing_at.0;
            ...
            if old_tx {
                debug!(
                    ...
                    tx_height_proved =% self.store.start_executing_at,
                    "Skipping old tx",
                );
                continue;
            }
...
mod tests {
    ...
    async fn setup() -> Result<(
        NodeState,
        RollupExecutor<TestContract>,
        Arc<NodeApiMockClient>,
    )> {
        ...
        let auto_prover = RollupExecutor::<TestContract>::build(bus.new_handle(), ctx).await?;
