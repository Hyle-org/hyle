...
use hyle_contract_sdk::{guest, ContractName, HyleOutput, ProgramInput};
...
impl transaction_builder::StateUpdater for CanonicalStates {
    ...
    fn execute(
        ...
        if contract_name == &self.hydentity_name {
            let (state, output) = guest::execute::<Hydentity>(program_input);
            Ok((Box::new(state) as Box<dyn std::any::Any>, output))
        } else if contract_name == &self.hyllar_name {
            let (state, output) = guest::execute::<Hyllar>(program_input);
            Ok((Box::new(state) as Box<dyn std::any::Any>, output))
        } else {
            anyhow::bail!("Unknown contract name: {contract_name}");
        }
