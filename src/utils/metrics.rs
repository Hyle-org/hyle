...
impl MempoolMetrics {
    pub fn global(conf: &Conf) -> MempoolMetrics {
        let my_meter = opentelemetry::global::meter(conf.metrics_namespace.clone());
    ...
    pub fn api_tx(&self, kind: String) {
        self.api_tx.add(1, &[KeyValue::new("kind", kind)]);
    }
    pub fn pending_tx(&self, nb: usize) {
        self.in_memory_tx
            .record(nb as u64, &[KeyValue::new("status", "pending")])
    }
    pub fn batched_tx(&self, nb: usize) {
        self.in_memory_tx
            .record(nb as u64, &[KeyValue::new("status", "batch")])
    }
    pub fn broadcasted_tx(&self, kind: String) {
