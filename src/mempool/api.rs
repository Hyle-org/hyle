...
use crate::{
    ...
    model::{
        BlobTransaction, CommonRunContext, Hashable, ProofTransaction, ProofTransactionB64,
...
pub async fn send_proof_transaction(
    State(state): State<RouterState>,
    Json(payload): Json<ProofTransactionB64>,
) -> Result<impl IntoResponse, AppError> {
    info!("Got proof transaction {}", payload.hash());
    let payload: ProofTransaction = payload
        .try_into()
        .map_err(|err: anyhow::Error| AppError(StatusCode::BAD_REQUEST, anyhow!(err)))?;
