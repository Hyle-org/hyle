...
pub struct MempoolStore {
    buffered_proposals: HashMap<ValidatorPublicKey, Vec<DataProposal>>,
    ...
    fn fetch_unknown_data_proposals(&mut self, cut: &Cut) -> Result<()> {
        ...
        for (validator, data_proposal_hash, _, _) in cut.iter() {
            if !self.lanes.contains(validator, data_proposal_hash) {
                let latest_data_proposal = self.lanes.lanes_tip.get(validator).cloned();
...
pub mod test {
    ...
    // TODO: Ajouter des tests sur les waiting dp ?
}
