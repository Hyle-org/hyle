...
impl Mempool {
    ...
    fn handle_data_proposal_management(&mut self) -> Result<()> {
        ...
        // 3. Go to latest DataProposal that has not reach PoA, and broadcast it to every validator that has not yet signed it
    ...
    fn handle_net_message(&mut self, msg: SignedByValidator<MempoolNetMessage>) -> Result<()> {
        ...
        if !self.validators.contains(validator) {
            bail!("Received message from unknown validator {:?}", validator);
        }
        ...
        match msg.msg {
            ...
            MempoolNetMessage::SyncReply(lane_entries) => {
                self
                    ...
                    .on_sync_reply(validator, lane_entries)?;
            }
        }
        Ok(())
    }
